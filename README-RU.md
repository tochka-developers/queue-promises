...
Условиями завершения промиса можно управлять более гибко. Для этого следует выполнить:
```php
$promise->setPromiseFinishConditions(
    true, // Выполнять промис в случае первого успешного завершения любой из задач 
    true  // Выполнять промис в случае возникновения первой же ошибки в любой из задач
);
```
Для удобства эти же параметры можно передать прямо в `runSync` или `runAsync`.
